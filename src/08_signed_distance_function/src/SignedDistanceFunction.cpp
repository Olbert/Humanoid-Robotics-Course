...
namespace signed_distance_function {
	...
	double SignedDistanceFunction::truncateDistance(const double& signedDistance, const double& delta) {
		double truncatedDistance = signedDistance;
	...
	void SignedDistanceFunction::integrateLaserScan(Eigen::MatrixXd & map, Eigen::MatrixXd & weights, const Measurement & measurement) {
		...
		{
			VectorOfPoints updatePoints = bresenham(measurement.robotPosition, measurement.laserPoints[i], map.rows(), map.cols());

			for (int j = 0; j < updatePoints.size(); j++)
			{
				double sd;
				if(j < updatePoints.size()/2 + 1)
					sd = -calculateDistance(updatePoints[j], measurement.laserPoints[i]);
				else
					sd = calculateDistance(updatePoints[j], measurement.laserPoints[i]);

				double tsd = truncateDistance(sd, delta);
				double weight = calculateWeight(tsd, delta, epsilon);

				if (weight < 1e-5)
					continue;

				int y = updatePoints[j](0);
				int x = updatePoints[j](1);
				map(x, y) = updateMap(tsd, weight, map(x, y), weights(x, y));
				weights(x, y) = updateWeight(weight, weights(x, y));
			}
		}
	}
	 ...
	 * \param[in] numRows The number of rows of the map.VectorOfPoints
